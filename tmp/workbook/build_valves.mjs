...
const outputDir =
  "/Users/erwannrousseau/Developer/guessit/outputs/019fcf61-98a2-74b1-b2af-9164c5c52bb0";
...
titleBand(
  summary,
  "A1:K1",
  "Registre centralisé - équipements du bateau",
  "A2:K2",
  `${sourceNote} La structure accepte de futurs instruments ou matériels, même si le PDF ne contient ici que des vannes.`,
);
...
summary.getRange("A9:C11").values = [
  ["OK", null, "Aucune action immédiate"],
  ["Inspection", null, "Programmer inspection"],
  ["À remplacer", null, "Planifier le remplacement"],
];
summary.getRange("B9").formulas = [["=COUNTIF('Équipements'!$I$5:$I$12,A9)"]];
summary.getRange("B9:B11").fillDown();
summary.getRange("A9:C11").format = {
  borders: { preset: "outside", style: "thin", color: colors.border },
  wrapText: true,
  verticalAlignment: "center",
};
summary
  .getRange("A9:A11")
  .conditionalFormats.add("containsText", { text: "OK", format: { fill: colors.green } });
summary
  .getRange("A9:A11")
  .conditionalFormats.add("containsText", { text: "Inspection", format: { fill: colors.orange } });
summary
  .getRange("A9:A11")
  .conditionalFormats.add("containsText", { text: "À remplacer", format: { fill: colors.red } });
...
summary.getRange("E9:G12").format = {
  borders: { preset: "outside", style: "thin", color: colors.border },
  wrapText: true,
  verticalAlignment: "center",
};
...
summary.getRange("A15:K15").format = {
  fill: colors.navy,
  font: { bold: true, color: "#FFFFFF" },
...
summary.getRange("A16:K18").values = [
  [
    ...
    "À remplacer",
    "Réservoir tribord",
    "Action prioritaire: planifier le remplacement.",
    null,
    null,
    null,
    null,
...
summary.getRange("A16:E18").format = {
  borders: { preset: "outside", style: "thin", color: colors.border },
  wrapText: true,
  verticalAlignment: "center",
};
...
summary.getRange("A20").values = [
  [
    "Limite de source: les observations sont génériques et ne donnent pas un historique distinct par identifiant; elles sont donc rattachées à toutes les vannes avec la mention Sections 1 à 5.",
  ],
];
...
titleBand(
  equipment,
  "A1:N1",
  "Équipements - registre détaillé",
  "A2:N2",
  "Une ligne par équipement. Les colonnes J (action) et les synthèses sont calculées à partir des valeurs saisies dans ce registre.",
);
...
equipment.getRange("J5").formulas = [
  [
    '=IF(I5="À remplacer","Planifier le remplacement",IF(I5="Inspection","Programmer inspection","Aucune action immédiate"))',
  ],
];
equipment.getRange("J5:J12").fillDown();
equipment.getRange("A5:N12").format = {
  borders: { preset: "outside", style: "thin", color: colors.border },
  verticalAlignment: "center",
};
...
equipment
  .getRange("I5:I12")
  .conditionalFormats.add("containsText", { text: "Inspection", format: { fill: colors.orange } });
equipment.getRange("I5:I12").conditionalFormats.add("containsText", {
  text: "À remplacer",
  format: { fill: colors.red, font: { bold: true } },
});
equipment
  .getRange("J5:J12")
  .conditionalFormats.add("containsText", { text: "remplacement", format: { fill: colors.red } });
equipment
  .getRange("J5:J12")
  .conditionalFormats.add("containsText", { text: "inspection", format: { fill: colors.orange } });
...
titleBand(
  observations,
  "A1:E1",
  "Observations du document source",
  "A2:E2",
  "Les cinq sections sont conservées séparément. Leur portée est générale: le PDF ne fournit pas de texte spécifique à un ID de vanne.",
);
observations.getRange("A4:E4").values = [
  ["Section", "Portée", "Texte centralisé", "Pages", "Fichier source"],
];
...
observations.getRange("A5:E9").format = {
  borders: { preset: "outside", style: "thin", color: colors.border },
  wrapText: true,
...
titleBand(
  ...
  "A2:E2",
  "Valeurs extraites du tableau des vannes. Les comptages se recalculent si le registre est complété avec d'autres équipements.",
);
reference.getRange("A4:E4").values = [
  ["Catégorie", "Valeur", "Type de référence", "Nombre", "Note"],
];
...
reference.getRange("A5:E12").values = systems.map((v) => [
  "Valve",
  v,
  "Système",
  null,
  "Extrait du champ Système",
]);
reference.getRange("D5").formulas = [["=COUNTIF('Équipements'!$C$5:$C$12,B5)"]];
reference.getRange("D5:D12").fillDown();
reference.getRange("A16:E23").values = materials.map((v) => [
  "Valve",
  v,
  "Matériau",
  null,
  "Extrait du champ Matériau",
]);
reference.getRange("D16").formulas = [["=COUNTIF('Équipements'!$G$5:$G$12,B16)"]];
reference.getRange("D16:D23").fillDown();
reference.getRange("A27:E29").values = statuses.map((v) => [
  "Valve",
  v,
  "État source",
  null,
  "Valeur de statut du PDF",
]);
...
titleBand(
  sources,
  "A1:B1",
  "Sources et notes d'audit",
  "A2:B2",
  "Traçabilité minimale de l'extraction et limites d'interprétation.",
);
...
sources.getRange("A5:B12").values = [
  ...
  [
    "Tableau principal",
    "Page 1: 8 vannes, colonnes ID, Système, Type, DN, PN, Matériau, Position, État.",
  ],
  ...
  [
    "Extension",
    "La colonne Catégorie permet d'ajouter d'autres équipements sans changer le modèle.",
  ],
];
sources.getRange("A5:B12").format = {
  borders: { preset: "outside", style: "thin", color: colors.border },
  wrapText: true,
  verticalAlignment: "top",
};
...
for (const sheetName of ["Synthèse", "Équipements", "Observations", "Référentiel", "Sources"]) {
  ...
  await fs.writeFile(
    `${previewDir}/${sheetName.replaceAll("é", "e").replaceAll("è", "e")}.png`,

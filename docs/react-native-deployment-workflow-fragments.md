...
The article should explain the decisions and use selected YAML excerpts as concrete evidence and a starting point for readers who want to reproduce the pipeline.

---

Apple Developer access, certificates, and GitHub secrets belong to one-time setup, not deployment. TestFlight is the real-device test stage. The human gate remains only for App Review submission: one fast, explicit GitHub action after the build has been tested.

---

Automation should remove repetitive release work, not the final decision to make an app public. The public-submission button is deliberately small, fast, and human.

---

A release is prepared when its tagged source has reached TestFlight. It is finalized only when the tested build is submitted to App Review. Those are distinct states, not two names for the same deploy.

---

The release pull request is the preparation space before the tag: one final verification, a visible changelog of what will ship, and a reviewable record of the exact release inputs.

---

Recovery preserves the release artifact: when the build already exists, submit that build rather than rebuild. A deployment retry should not silently create a different binary.

---

The release begins with a small human gesture: open GitHub Actions, enter the version and release notes, and let the workflow create the release pull request. The system turns that intent into the reviewed, tagged source that EAS builds.

---

Write the article in French first. Translation can come later.

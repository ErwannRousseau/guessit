...
GuessIt uses GitHub Flow with a protected `main`, short-lived branches, required pull-request CI, and squash merges. GitHub Actions owns deterministic release preparation from manually supplied version and release-note inputs, then a merged release pull request produces the shared Git tag and draft GitHub Release that gate the EAS iOS build and TestFlight upload. App Review submission remains an explicit GitHub Actions operation after TestFlight verification; a Codex skill would duplicate this repository-owned automation and is intentionally omitted.
